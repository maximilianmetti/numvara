function D = radialDarcyDiss(x,xprev,xhat,dt,f)
%%  Evaluate the dissipation given by Darcy's law for the FE flow map
N  = length(x)-1;
D  = @(x,xp,xhat,dt,f) xhat.*f.*(x-xp).^2/(dt*dt);

if length(x)-length(xprev)~=0
    display('dimension mismatch in dissipation');
end
if length(x)-length(xhat)~=0
    display('dimension mismatch in dissipation');
end
xm    = .5*(x(1:N)+x(2:N+1));
...
D = dxhat'*(   D(x(1:N),xprev(1:N),xhat(1:N),dt,f(xhat(1:N)))...
            +4*D(xm,xpm,xhatm,dt,f(xhatm))...
            +  D(x(2:N+1),xprev(2:N+1),xhat(2:N+1),dt,f(xhat(2:N+1))) )/6;
